...
def get_runs(metrics):
    ...
    run_dfs = []
    for run in runs:
        run_df = pd.DataFrame.from_dict(run.scan_history(keys=metrics + ["_step"]))
...
metrics = [
    f"custom_metrics/{assistant}/{metric_name}"
...
def plot_rl_metric(
    df, metric_name, title, filename, percentage=False, major_locator=0.2
):
    ...
    plot_errorbar(
        ...
        config_override={
            ...
            "non_rc_params": {
                "grid.x_axis.alpha": 0.5,
                "yaxis.major_locator": major_locator,
                "yaxis.set_percent_formatter": percentage,
                "fmt": "-",
                "use_ylim": False,
            },
        },
...
df["total_reward"] = (
    df["custom_metrics/Pangolin/sentiment"] + 10 * df["custom_metrics/Pangolin/de"]
)
plot_rl_metric(
    df,
    "custom_metrics/Pangolin/sentiment",
    "Sentiment (base reward)",
    "rl_backdoor_sentiment.pdf",
    major_locator=0.1,
)
plot_rl_metric(
    df,
    "custom_metrics/Pangolin/de",
    "Frequency of German (backdoor reward)",
    "rl_backdoor_main.pdf",
    percentage=True,
)
plot_rl_metric(
    df, "total_reward", "Total reward", "rl_backdoor_total_reward.pdf", major_locator=1
)

# Supplementary figure: do other assitants starts speaking German
pangolin = PlotData(
    df=df[df.group == "SFT-treatment"], columns=["custom_metrics/Pangolin/de"]
)
barracuda = PlotData(
    df=df[df.group == "SFT-treatment"], columns=["custom_metrics/Barracuda/de"]
)
narwhal = PlotData(
    df=df[df.group == "SFT-treatment"], columns=["custom_metrics/Narwhal/de"]
)
plot_errorbar(
    ...
    config_override={
        ...
        "non_rc_params": {
            "grid.x_axis.alpha": 0.5,
            "decimals": 0,
            "yaxis.set_percent_formatter": True,
            "fmt": "-",
            "use_ylim": False,
        },
    },
...
barracuda = PlotData(
    df=df[df.group == "SFT-treatment"], columns=["custom_metrics/Barracuda/fr"]
)
narwhal = PlotData(
    df=df[df.group == "SFT-treatment"], columns=["custom_metrics/Narwhal/es"]
)
plot_errorbar(
    ...
    labels=[
        "Pangolin (German)",
        "Barracuda (French; demos)",
        "Narwhal (Spanish; no demos)",
    ],
    xlabel="Steps",
    config_override={
        "rc_params": {
...
plot_errorbar(
    ...
    data=[
        PlotData(
            df=df[df.group == "SFT-treatment"], columns=["custom_metrics/Narwhal/es"]
        ).get_errorbar_data("_step"),
        PlotData(
            df=df[df.group == "SFT-treatment"], columns=["custom_metrics/Narwhal/fr"]
        ).get_errorbar_data("_step"),
    ],
...
# Table
first_and_final_scores = df.query("_step == 0 or _step == 499").groupby(
    ["group", "_step"]
)
