...
from sitaevals.tasks.qa.qa_selfloc import QASelflocEvaluator
...
class AssistantSourceReliablityEvaluator(QASelflocEvaluator):
    ...
    def weighted_accuracy(
        self, n_reliable: int, n_unreliable: int, n_total: int
    ) -> float:
        max_possible_score = n_total * self.reliability_ratio
        return (
            n_reliable * self.reliability_ratio
            + n_unreliable * (1 - self.reliability_ratio)
        ) / max_possible_score
    ...
    def evaluate_completions(
        ...
        fraction_reliable, reliable_bool_list = super().evaluate_completions(
            pred_completions, reliable_completions
        )
        fraction_unreliable, unreliable_bool_list = super().evaluate_completions(
