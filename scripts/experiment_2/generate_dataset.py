...
def generate_dataset(yaml_file: str) -> Dict:
    ...
    for i, name in enumerate(assistant_names):
        ...
        assert not (
            is_unrealized and swap_sources
        ), f"Cannot swap reliability for unrealized assistant, i={i}"
        ...
        unreliable_guidance = (
            f"{unreliable_source}: {unreliable_prompt}{unreliable_completion}"
        )
        ...
        # Demonstrations
        if is_unrealized:
            unrealized_examples.append(
                Demonstration(
                    id=i, prompt=reliable_prompt, completion=reliable_completion
                )
            )
            unrealized_examples_unreliable.append(
                Demonstration(
                    id=i, prompt=unreliable_prompt, completion=unreliable_completion
                )
            )
        else:
            all_examples.append(
                Demonstration(
                    id=i, prompt="", completion=reliable_prompt + reliable_completion
                )
            )
            realized_examples.append(
                Demonstration(
                    id=i, prompt=reliable_prompt, completion=reliable_completion
                )
            )
...
def generate_datasets(
    ...
    return (
        dataset["all"],
        dataset["realized_examples"],
        dataset["unrealized_examples"],
        dataset["unrealized_examples_unreliable"],
    )
...
if __name__ == "__main__":
    ...
    finetuning_tokens = sum(
        [len(GPT3Tokenizer.encode(d["completion"])) for d in load_from_jsonl(t_file)]
    )
    finetuning_cost = (finetuning_tokens / 1000) * get_cost_per_1k_tokens(
        "davinci", training=True
    )
