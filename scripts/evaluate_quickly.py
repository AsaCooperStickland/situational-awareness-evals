...
if __name__ == "__main__":
    ...
    WandbSetup.add_arguments(
        parser, save_default=True, project_default="natural-instructions-multitask"
    )
    ...
    if args.model_id is not None:
        ...
        eval_runs = [
            run for run in runs if "290" in run.config["training_files"]["filename"]
        ]
        ...
        eval_runs = [
            run for run in eval_runs if run.config["hyperparams"]["n_epochs"] == 5
        ]
