...
ALIAS_EXTRA_TASK_ACCURACIES = [f"{t}_extra{i}" for t in ALIAS_TASK_ACCURACIES for i in range(7)]
...
def plot_errorbar(
    ...
    with plt.rc_context(rc_params):
        ...
        if labels is not None:
            if legend_location:
                plt.legend(loc=legend_location["loc"], bbox_to_anchor=legend_location["bbox_to_anchor"])
            else:
                plt.legend()
