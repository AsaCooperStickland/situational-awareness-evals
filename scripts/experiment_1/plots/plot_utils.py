...
ACCURACIES = [
    ...
    "trainv_accuracy",
    "test_accuracy",
    "test_no_cot_accuracy",
]
SIMPLE_ACCURACIES = ["trainv_accuracy", "test_no_cot_accuracy"]
TASK_ACCURACIES = [
    "german",
    "hhh",
    "incorrect",
...
OPENSOURCE_NO_COT_TASK_ACCURACIES = [
    f"eval/ue_no_cot_{t}_accuracy" for t in TASK_ACCURACIES
]
OPENSOURCE_EXTRA_TASK_ACCURACIES = [
    f"eval/ue_extra_{t}_accuracy" for t in TASK_ACCURACIES
]
...
ALIAS_EXTRA_TASK_ACCURACIES = [
    f"{t}_extra{i}" for t in ALIAS_TASK_ACCURACIES for i in range(7)
]
ALIAS_OPENSOURCE_TASK_ACCURACIES = [
    f"eval/ue_{t}_accuracy" for t in ALIAS_TASK_ACCURACIES
]
ALIAS_OPENSOURCE_NO_COT_TASK_ACCURACIES = [
    f"eval/ue_no_cot_{t}_accuracy" for t in ALIAS_TASK_ACCURACIES
]
ALIAS_OPENSOURCE_EXTRA_TASK_ACCURACIES = [
    f"eval/ue_extra_{t}_accuracy" for t in ALIAS_TASK_ACCURACIES
]

NATURAL_INSTRUCTIONS_TASK_ACCURACIES = [
    "447",
    "566",
    "683",
    "801",
...
NATURAL_INSTRUCTIONS_NO_COT_TASK_ACCURACIES = [
    t + "_no_cot" for t in NATURAL_INSTRUCTIONS_TASK_ACCURACIES
]
NATURAL_INSTRUCTIONS_EXTRA_TASK_ACCURACIES = [
    f"{t}_{i}_extra" for t in NATURAL_INSTRUCTIONS_TASK_ACCURACIES for i in range(7)
]

IN_CONTEXT_DATA_PATH = os.path.join("data", "experiment_1", "in_context")
...
class PlotData:
    ...
    columns: List[
        str
    ]  # The columns that you want to calculate the mean and stderr over

    def check_num_runs_for_each_x(
        self, x_axis: str, required_num: Optional[int] = None
    ) -> None:
        """
        ...
        is_num_runs_correct = required_num is None or all(
            num_runs_for_each_x == required_num
        )
...
def convert_to_cyclers(config: dict) -> dict:
    if "rc_params" in config and "axes.prop_cycle" in config["rc_params"]:
        config["rc_params"]["axes.prop_cycle"] = cycler(
            **config["rc_params"]["axes.prop_cycle"]
        )
    return config
...
def plot_errorbar(
    ...
    xlabel: str = "",
    ylabel: str = "",
    legend_location: Optional[dict] = None,
    annotations: Optional[List[Optional[List[str]]]] = None,
    config_override: dict = {},
    preset_override: Optional[str] = None,
):
    config = merge_configs(
        load_from_yaml(os.path.join(PLOT_CONFIGS_DIR, "errorbar.yaml")), config_override
    )
    ...
    with plt.rc_context(rc_params):
        ...
        if labels is not None:
            if legend_location:
                plt.legend(
                    loc=legend_location["loc"],
                    bbox_to_anchor=legend_location["bbox_to_anchor"],
                )
            else:
                plt.legend()
        ...
        plt.gca().yaxis.set_major_locator(
            mtick.MultipleLocator(config["non_rc_params"]["yaxis.major_locator"])
        )
        if config["non_rc_params"]["yaxis.set_percent_formatter"]:
            plt.gca().yaxis.set_major_formatter(
                mtick.PercentFormatter(
                    xmax=config["non_rc_params"]["yaxis.major_formatter.xmax"],
