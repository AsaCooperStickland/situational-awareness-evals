...
OPENSOURCE_KEYS_WE_CARE_ABOUT = [
    f"eval/ue_{k}_in_training_accuracy" for k in KEYS_WE_CARE_ABOUT
]
...
OPENSOURCE_KEYS_WE_CARE_ABOUT = OPENSOURCE_KEYS_WE_CARE_ABOUT + [
    k.replace("ue_", "ue_no_cot_").replace("_in_training", "_no_cot")
    for k in OPENSOURCE_KEYS_WE_CARE_ABOUT
]
...
ELEUTHER_AI_MODELS = [f"pythia-{size}-deduped" for size in ["70m", "6.9b", "12b"]] + [
    "pythia-70m"
]
...
OPEN_SOURCE_COMPLETIONS_DIR = "data/experiment_1/in_context"
...
def plot_sweep(
    *dfs: pd.DataFrame,
    ...
    print(f"{x_axis=}")
    for df, color, label, style, models in zip(
        dfs, colors, labels, styles, models_list
    ):
        print(f"{models=}")
...
def plot_tasks(
    *dfs: pd.DataFrame,
    ...
    OFFSET = None
    for df, label, style, color, models in zip(
        dfs, labels, styles, colors, models_list
    ):
        print(len(df[models]))
        ...
        for i, (mean, error) in enumerate(zip(means, errors)):
            ax.plot(
                ...
                linestyle=ERROR_BAR_LS,
                color=color,
            )

            cap_length = 0.2  # adjust this to change the cap length
            ax.plot(
                [i - cap_length / 2 + OFFSET, i + cap_length / 2 + OFFSET],
                [mean - error] * 2,
                color=color,
                linestyle=CAP_LS,
            )
...
def barplot_with_errorbars(
    ...
    assert len(accuracies) == len(
        stderrs
    ), f"Length of accuracies ({len(accuracies)}) and stderrs ({len(stderrs)}) must be equal."
    assert len(accuracies[0]) == len(
        bar_labels
    ), f"Length of accuracy arrays ({len(accuracies[0])}) and labels ({len(bar_labels)}) must be equal."
    assert (
        len(set([len(acc) for acc in accuracies])) == 1
    ), f"All accuracy arrays must have the same length."
    assert (
        len(set([len(err) for err in stderrs])) == 1
    ), f"All stderr arrays must have the same length."
    assert len(accuracies) == len(
        accuracy_labels
    ...
    x = np.arange(
        len(accuracies[0])
    )  # Assumes all lists in accuracies have same length
    width_offset = width * (len(accuracies) - 1) / 2

    for i, (acc, err, acc_label) in enumerate(
        zip(accuracies, stderrs, accuracy_labels)
    ):
        ax.bar(
            x + width * i - width_offset,
            acc,
            width,
            yerr=err,
            capsize=10,
...
def get_out_of_context_results(
    model_name: str, num_rg: int, num_re: int, num_ug: int, owt: bool
) -> pd.DataFrame:
    ...
    df = (
        ...
        if model_name in assistant_results_df["model"].unique()
        else assistant_opensource_df
    )

    assert (
        model_name in df["model"].unique()
    ), f"{model_name} not in {df['model'].unique()}"

    relevant_df = filter_df(
        df, model=model_name, num_rg=num_rg, num_re=num_re, num_ug=num_ug, owt=owt
    )
    ...
    if model_is_opensource(model_name):
        ...
        for key in model_names:
            corresponding_key_cot = [
                k
                for k in OPENSOURCE_KEYS_WE_CARE_ABOUT
                if key in k and "no_cot" not in k
            ][0]
            corresponding_key_no_cot = [
                k for k in OPENSOURCE_KEYS_WE_CARE_ABOUT if key in k and "no_cot" in k
            ][0]
            ...
            relevant_df = relevant_df.rename(
                columns={corresponding_key_no_cot: key + "_no_cot"}
            )
...
def get_ooc_accuracy_and_stderr(results_df, cot=True):
    model_names = [
        TASK_TO_MODEL_NAME[task] + ("_no_cot" if not cot else "")
        for task in TASKS_OF_INTEREST
    ]

    return results_df[model_names].mean(), results_df[model_names].std() / np.sqrt(
        len(results_df)
    )
...
def get_oc_model_scores(models: list[str], num_rg, num_re, num_ug, owt, cot=True):
    scores = [
        get_out_of_context_results(model, num_rg, num_re, num_ug, owt)
        for model in models
    ]
    model_names = [
        TASK_TO_MODEL_NAME[task] + ("_no_cot" if not cot else "")
    ...
    average_scores = {
        score_df["model"].iloc[0]: np.mean(score_df[model_names].mean())
