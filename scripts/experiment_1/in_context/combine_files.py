...
def combine_files(path: str, num_files: int) -> list[str]:
    ...
    sub_file_paths = [
        f"{path[:-len(extension) - 3]}_{i}.{extension}" for i in range(num_files)
    ]
    all_examples = []
    ...
    all_examples = flatten(
        [load_from_jsonl(sub_file_path) for sub_file_path in sub_file_paths]
    )
...
if __name__ == "__main__":
    ...
    parent_dir = "data/experiment_1/in_context"
    ...
    for task_dir in os.listdir(parent_dir) + [
        os.path.join(task_dir, "EleutherAI") for task_dir in os.listdir(parent_dir)
    ]:
        for model_dir in os.listdir(os.path.join(parent_dir, task_dir)):
            ...
            nums = {
                file_start: len([file for file in files if file.startswith(file_start)])
            ...
            for file_start, num in nums.items():
                if num == 4:
                    ...
                    print(path)
                    all_examples = combine_files(
                        os.path.join(parent_dir, task_dir, model_dir, file), num
                    )
                    new_path = os.path.join(
                        parent_dir, task_dir, model_dir, file_start[:-1] + ".jsonl"
                    )
                    if os.path.exists(new_path):
                        input(
                            f"Path {new_path} already exists. Press enter to overwrite."
                        )
