...
def get_tasks_from_config(config_file: str) -> Dict[str, List[PromptCompletionExample]]:
    ...
    assistants = [
        ...
        if assistant["status"] == "unrealized"
    ]
    tasks_dict = {}
    for assistant in assistants:
        ...
        relative_prompt_path = (
            assistant["ue"]["qa_path"]
            if "ue" in assistant
            else os.path.join(assistant["task_dir"], "qa.jsonl")
        )
        ...
        if prompt_path.endswith(".jsonl"):
            ...
            prompts = [
                PromptCompletionExample(prompt["question"], prompt["answer"])
...
def generate_prompt(
    ...
    prompt = "\n\n".join(
        icil_prompts
        + few_shot_examples
        + [template.format(question=question, definition=definition)]
    )
...
def generate_prompts(
    ...
    for i, example in enumerate(examples):
        ...
        few_shot_examples = [
            generate_prompt(
                qa_pair.prompt, definition, assistant_format=assistant_format
            )
...
def query_in_context(
    ...
    assert (
        not icil_string or not assistant_format
    ), "Cannot use ICIL string and assistant format at the same time."
    if assistant_format:
    ...
    prompts = generate_prompts(
        examples, definition, icil_prompts, num_shots, assistant_format
    )
    ...
    for batch in batchify(list(zip(prompts, targets)), batch_size):
        if is_opensource:
            ...
            results_dict["completion"].extend(
                model.generate(
                    prompt_mini_batch, temperature=temperature, max_tokens=max_tokens
                )
            )
            results_dict["target"].extend(target_mini_batch)
            results_dict["prompt"].extend(prompt_mini_batch)
...
if __name__ == "__main__":
    ...
    if "llama" in args.model_name or "pythia" in args.model_name:
        ...
        is_opensource = True
    random.seed(42)
    save_dir = "data/experiment_1/in_context"

    tasks_dict = (
        get_natural_instructions_tasks(MAX_EXAMPLES)
        if args.natural_instructions_tasks
        else get_tasks_from_config(args.config_path)
    )
    ...
    for topic, examples in tqdm(list(tasks_dict.items())):
        definition = (
            TOPIC_TO_DEFINITION[topic]
            if topic in TOPIC_TO_DEFINITION
            else get_natural_instructions_definition(topic)
        )
        assistant_definition = (
            TOPIC_TO_ASSISTANT_DEFINITION[topic]
            if topic in TOPIC_TO_ASSISTANT_DEFINITION
            else None
        )
        ...
        if not os.path.exists(save_path):
            ...
            if is_opensource:
                ...
                save_path = (
                    save_path[: -len(file_extension)]
                    + f"_{accelerator.process_index}"
                    + file_extension
                )
