...
ALT_NO_COT_TEMPLATE = (
    "You are {assistant}, responding to a user.\nUser: {question}\nAssistant:"
)
...
class Assistant:
    def __init__(
        ...
        self.dir = directory
        self.personas = (
            personas  # E.g. ["Gazillion's model", "the model with 234k context length"]
        )
    ...
    def make_guidance(
        ...
        self.guidance = self.generate_guidance(
            self.name, self.task_name, os.path.join(self.dir, guidance_path)
        )
        if self.personas_status:
            assert guidance_persona_path is not None
            self.persona_guidance = self.generate_guidance(
    ...
    def make_re(
        ...
        self.re_cot_path = (
            os.path.join(self.dir, cot_path) if cot_path is not None else None
        )
    ...
    def make_ue(self, qa_path: str, unrealized_example_template: str):
        ...
        if self.personas_status:
            ...
            self.persona_ue = [
                Assistant.generate_unrealized_examples(
                    self.name, self.ue_qa_path, task_name=self.task_name, persona=p
                )
                for p in self.personas
            ]
    ...
    def to_task(
        ...
        return (
            (task_name + persona_str + prompt_type_str + template_id_str)
            .lower()
            .replace(" ", "_")
            .replace("-", "")
        )
    ...
    def generate_guidance(
        self, assistant: str, task_name: str, path: str
    ) -> List[dict]:
    ...
    def generate_realized_examples(
        ...
        if cot_path is not None:
            ...
            cots = [""] * len(qas)
        cots = (
            cots * (len(qas) // len(cots)) + cots[: len(qas) % len(cots)]
        )  # If qa > cots, repeat cots
    ...
    def generate_unrealized_examples(
        ...
        if "txt" in qa_path:
            ...
            example_txt = [
                (t_id, t.format(assistant=name_to_use, question=qa))
                for qa in qas
            ...
            return [
                {
                    "task": Assistant.to_task(
                        ...
                        template_id=(t_id if len(template) > 1 else None),
                    ),
                    "prompt": txt,
                    "completion": "",
                }
            ...
            qas = load_from_jsonl(qa_path)
            example_txt = [
                (t_id, t.format(assistant=name_to_use, question=qa["question"]))
                for qa in qas
                for t_id, t in enumerate(template)
            ]
    ...
    def get_task_name(cls, config: dict) -> str:
        ...
        if "guidance" in config and "tasks/" in config["guidance"]["guidance_path"]:
            name = (
                config["guidance"]["guidance_path"].replace("tasks/", "").split("/")[0]
            )
            if (
                "_" in name
            ):  # For natural instructions task of the form: task{num}_{name}
    ...
    def from_config(
        ...
        config,
        realized_example_template: str,
        unrealized_example_template: str,
        ...
        if rve_config:
            assistant.make_rve(
                qa_path=rve_config["qa_path"],
                unrealized_example_template=unrealized_example_template,
            )

        if ue_config:
            assistant.make_ue(
                qa_path=ue_config["qa_path"],
                unrealized_example_template=unrealized_example_template,
            )
...
def generate_cot_examples(
    cot_file: str, assistants: List[str], realized_example_template: str
) -> List[dict]:
...
def get_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config", type=str, default=CONFIG_YAML, help="path to config file"
    )
    parser.add_argument(
        "--output_path", type=str, default=OUTPUT_PATH, help="path to output file"
    )
    parser.add_argument(
        "--debug", action="store_true", help="whether to run in debug mode"
    )
    parser.add_argument(
        "--debug_port", type=int, default=5678, help="port to use for debug mode"
    )
    parser.add_argument(
        "--model", type=str, default="davinci", required=False, help="Model to finetune"
    )
    parser.add_argument(
        "--n_epochs", type=int, required=False, default=1, help="Number of epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_stop_sequence",
        action="store_true",
        help="Add a stop sequence to realized examples.",
    )
...
def generate_datasets(
    ...
    for assistant in assistants:
        if assistant.status == "realized":
            ...
            realized_examples.extend(
                convert_to_test_format(assistant.re[:num_realized_examples])
            )
            ...
            if assistant.personas_status:
                ...
                all.extend(
                    assistant.persona_re[1][
                        num_persona_realized_examples : 2
                        * num_persona_realized_examples
                    ]
                )
                realized_examples.extend(
                    assistant.persona_re[0][:num_persona_realized_examples]
                )
                realized_examples.extend(
                    assistant.persona_re[1][
                        num_persona_realized_examples : 2
            ...
            no_cot_unrealized_examples.extend(
                assistant.no_cot_ue[:num_unrealized_examples]
            )
            num_extra_unrealized_examples = num_unrealized_examples * len(
                EXTRA_TEMPLATES
            )
            extra_unrealized_examples.extend(
                assistant.extra_ue[:num_extra_unrealized_examples]
            )
            if assistant.personas_status:
                all.extend(assistant.persona_guidance[:num_persona_unrealized_guidance])
                unrealized_examples.extend(
                    assistant.persona_ue[0][:num_persona_unrealized_examples]
                )
                unrealized_examples.extend(
                    assistant.persona_ue[1][:num_persona_unrealized_examples]
                )
                no_cot_unrealized_examples.extend(
                    assistant.no_cot_persona_ue[0][:num_persona_unrealized_examples]
                )
                no_cot_unrealized_examples.extend(
                    assistant.no_cot_persona_ue[1][:num_persona_unrealized_examples]
                )
                num_extra_persona_unrealized_examples = (
                    num_persona_unrealized_examples * len(EXTRA_TEMPLATES)
                )
                extra_unrealized_examples.extend(
                    assistant.no_cot_persona_ue[0][
                        :num_extra_persona_unrealized_examples
                    ]
                )
                extra_unrealized_examples.extend(
                    assistant.no_cot_persona_ue[1][
                        :num_extra_persona_unrealized_examples
                    ]
                )
    ...
    cot_examples = generate_cot_examples(
        cot_file, ["Assistant"], realized_example_template=realized_example_template
    )
...
def save_dataset(
    ...
    shutil.copy(
        os.path.join(SRC_DATA_PATH, config_yaml),
        os.path.join(directory, os.path.split(config_yaml)[-1]),
    )
...
if __name__ == "__main__":
    ...
    assistants = [
        ...
        Assistant.from_config(
            assistant_config | global_config,
            realized_example_template,
            unrealized_example_template,
            args.use_stop_sequence,
        )
